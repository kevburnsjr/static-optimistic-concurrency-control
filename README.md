...
In traditional OCC, a transaction is executed and submitted for commit with assumed isolation; that the process
...
cache to be updated to their latest version simultaneously, optimistically downgrading the transaction to a *static*
data access scheme.
...
modified records need to be refreshed on retry rather than re-fetching the entire read set.
